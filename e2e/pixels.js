...
export async function scanRegion(page, selector, { exclude = [] } = {}) {
  ...
  return page.evaluate(
    async ({ dataUrl, skip }) => {
      ...
      for (let y = 0; y < canvas.height; y += 1) {
        for (let x = 0; x < canvas.width; x += 1) {
          if (skip.some((s) => x >= s.x && x < s.x + s.width && y >= s.y && y < s.y + s.height)) continue;
    ...
    { dataUrl: `data:image/png;base64,${shot.toString('base64')}`, skip },
  );
}

export async function assertOpaque(page, selector, { expected, exclude = [], tolerance = 6 } = {}) {
  ...
  if (foreignCount > 0) {
    ...
    throw new Error(
      `assertOpaque: ${selector} is not opaque — ${foreignCount}/${total} px differ from ` +
        `rgb(${expected.join(',')}). Content behind it is showing through. Worst offenders: ${worst}`,
    );
  }
  return { total, foreignCount };
}
